...
#
####################################
echo "Disable internal wifi? (y|n):"
read SHOULD_DISABLE_WIFI

if [ "$SHOULD_DISABLE_WIFI" = "y" ]; then
   # not sure if it only works for pi3
   echo "dtoverlay=pi3-disable-wifi >> /boot/config.txt"
   "# disable internal wifi" >> /boot/config.txt
   "dtoverlay=pi3-disable-wifi" >> /boot/config.txt
else
   echo "skipping disable wifi"
fi
